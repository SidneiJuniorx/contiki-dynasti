...
#ifndef PERIOD
#define PERIOD 60*5 // Sidnei alterou de 60 para 10
#endif
...
PROCESS(instance_process20, "Evento Esporadico 20");
...
void instance_20(clock_time_t event) {
  ...
  printf("Instancia 20 Teste\n");
  process_start(&instance_process20, (void *)&event);
}
...
/********************************************************************************/
PROCESS_THREAD(instance_process20, ev, data)
{
...
/*****************************************************************/
  ...
  while(1) {
    ...
    if(etimer_expired(&periodic)) {
      ...
      if(status == 1)
      {
        ctimer_set(&backoff_timer, SEND_TIME, send_packet20, NULL);
      }
    }
