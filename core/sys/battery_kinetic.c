...
unsigned day_profile=7; // Change it if you want to start with a different day
short new_day=1;
double total_consumption=0, periodic_consumption=0;

/*-------------------------ELT and EHARM Variables------------------------------*/
double ELT = 1;
double EHARM = 1;
/*-------------------------Residual Energy Variable-----------------------------*/
//double RE = 100;
/*-------------------------Drain Rate Variables---------------------------------*/
double new_drain_rate;
double drain_rate = 1;
double old_drain_rate = 1;
double min_drain_rate = 30;
double interval_consumption = 0;
double alfa = 0.3;
int set_state;
/*------------------------------------------------------------------------------*/
double solar_charging_func;
/*------------------------------------------------------------------------------*/
...
void battery_start(clock_time_t period, unsigned seconds, double fixed_perc_energy, unsigned variation, int state_client, double batt_perc) {
	...
	batt.qmax = 1*50000; //Sidnei alterou de 1000000 para 50000

	/* Trecho de código adicionado por Sidnei*/
	battery_qmax = batt.qmax * batt_perc;
	batt.q1_0 = 1*battery_qmax * random_variable; //Sidnei alterou de 1000000 para 50000
	batt.q2_0 = battery_qmax * (1 - random_variable) ; // q1 and q2 should start balanced
        //batt.q2_0 = 0;
	/*Fim do trecho*/

	//batt.q1_0 = 1*50000 * random_variable; //Sidnei alterou de 1000000 para 50000
	...
	set_state = state_client;
	//random_battery1 = random_battery1 + 1;
	//printf("Random Battery is: %d \n",random_battery1);
...
void update_battery(){
  ...
  int stt_couter, num_states=set_state;//set_state
...
//-------- call the Kinetic model -----------------
  for(stt_couter=0;stt_couter<num_states;stt_couter++){
	...
	printf("Log For: Energy Periodic consumption (microA); %d;  \n",(int) (periodic_consumption));
	interval_consumption += periodic_consumption;
  }
	...
	//printf("Battery: Total of battery q1 + q2 = qmax: %lu \n",(unsigned long)(batt.qmax));
	//printf("Energy: CPU ticks %lu, LPM %lu, Tx %lu, Rx %lu \n", stats_com.cpu, stats_com.lpm, stats_com.transmit, stats_com.listen);

  	/*********Cálcula Drain Rate e min Drain Rate ******/
	//printf("Drain Rate: The OLD Drain Rate is: %d \n",(unsigned long) (old_drain_rate));
	
	new_drain_rate = interval_consumption; //Consumption on 5 minutes
	//printf("Drain Rate: The NEW Drain Rate is: %d \n",(unsigned long) (new_drain_rate));
	drain_rate = old_drain_rate * alfa + (1 - alfa) * new_drain_rate;
	//printf("Drain Rate: The EWMA Drain Rate is: %d \n",(unsigned long) (drain_rate));
	old_drain_rate = drain_rate;
	
	if (min_drain_rate > drain_rate){
		min_drain_rate = drain_rate;
		//printf("Min_drain_rate is = %d \n",(unsigned long) (min_drain_rate));
	}
	
	//printf("Drain Rate: The Interval Consumption (microAH): %lu \n",(unsigned long) (interval_consumption));
	interval_consumption = 0; //Reset the interval consumption
	/****************************************************************/
	if (set_state == 4) solar_charging_func = 0;
	if (set_state == 5) solar_charging_func = solar_charging;
	
	
	printf("Solar Charging: The charge on the interval is: %d \n",(unsigned) solar_charging_func);
	printf("Numbers of states on battery: %d \n", set_state);

	/*--------------------------------------------------------------*/

	/*********Cálcula ELT e ELTmin **********************************/
	/*
	ELT = (((batt.q1_0+batt.q2_0)/batt.qmax) * 1000) / drain_rate;
	...
	*/
	...
	/*-------------------------Cálcula EHARM------------------------*/
	//ELT = (((batt.q1_0+batt.q2_0)/batt.qmax) * 1000) / drain_rate;
	//EHARM = ELT + ((-solar_charging_func/batt.qmax) * 1000);
	//printf("EHARM: %d \n",(unsigned long) EHARM);

	/*--------------------------------------------------------------*/

	/*-------------------------Cálcula Residual Energy--------------*/
	//RE = ((batt.q1_0+batt.q2_0)/batt.qmax) * 100;
	//printf("Battery is: %d \n",(unsigned long) RE);
	/*--------------------------------------------------------------*/
}
